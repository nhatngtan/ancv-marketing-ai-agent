...
import { createBridgeToken, dedicatedFlowProfilePath, localAgentConfigPath } from './config.js';
...
export function configureLocalAgent(workspaceRoot = process.env.ANCV_LOCAL_WORKSPACE || 'D:\\ANCV Marketing'): string {
  ...
  const expectedAccount = process.env.ANCV_FLOW_ACCOUNT_EMAIL
    || previous.profiles?.find((profile) => profile.logicalId === 'flow-gold')?.expectedAccount
  ...
  const config = {
    ...
    profiles: [{ logicalId: 'flow-gold', kind: 'managed', userDataDir: dedicatedFlowProfilePath('gold'), ...(expectedAccount ? { expectedAccount } : {}) }],
  };
