...
| Website | REST/Auth PASS; đúng 01 media upload + đúng 01 draft UAT PASS (Post `801`, Media `800`); Yoast write field không được expose | Publish/schedule/update/delete NOT TESTED; API trả role `administrator` khác role `Editor`; Yoast `NOT_SYNCED_TO_YOAST` | Semi-automatic |
