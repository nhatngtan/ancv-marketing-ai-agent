  ...
  it('uses GET only and reports authenticated read without testing write capabilities', async () => {
    ...
    const mockFetch = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.includes('/wp-json/wp/v2/users/me')) {
        ...
        return new Response(JSON.stringify({ id: 7, slug: 'editor01', roles: ['editor'] }), { status: 200, headers: { 'content-type': 'application/json' } });
      }
    ...
    expect(requestMethods).toEqual(['GET', 'GET', 'GET']);
    expect(result).toMatchObject({
      ...
      evidence: { publicReadAccess: 'passed', usersMeRequest: 'passed', readAccess: 'passed', writeAccess: 'not_tested', mediaUpload: 'not_tested', publishing: 'not_tested' },
    });
  });
});
