...
const profileStatusLabels = { ready:'Sẵn sàng',login_required:'Cần đăng nhập',bridge_required:'Browser Bridge chưa sẵn sàng',unavailable:'Không truy cập được',not_tested:'Chưa kiểm tra' } as const;
...
export function CompanySettings({onToast}:{onToast:(text:string)=>void}) {
  ...
  return <>
    ...
    {admin&&<section className="panel company-settings browser-profile-settings">
      ...
      <div className="channel-profile-grid"><strong>Kênh & Profile</strong>{BROWSER_PLATFORMS.map((platform)=>{const mapping=browser?.settings?.mappings?.[platform];const validation=browser?.settings?.validations?.[platform];const platformStatus=platform==='google_flow'&&validation?.platformStatus==='ready_for_write_test'?'Sẵn sàng':validation?.platformStatus?platformStatusLabels[validation.platformStatus]:'Chưa cấu hình';return <div className="channel-profile-row" key={platform}><label>{platformLabels[platform]}<select value={selections[platform]??''} onChange={(event)=>setSelections({...selections,[platform]:event.target.value||undefined})}><option value="">Chọn Chrome Profile</option>{browser?.settings?.profiles?.map((item)=><option value={item.chromeProfileId} key={item.chromeProfileId}>{item.chromeProfileId} — {item.profileLabel}{item.email?` — ${item.email}`:''}</option>)}</select></label><div className="profile-validation"><span>{validation?.profileStatus?profileStatusLabels[validation.profileStatus]:'Chưa kiểm tra'}</span><small>{platformStatus}</small></div><button className="secondary" disabled={!agentOnline||Boolean(browserBusy)||!mapping||selections[platform]!==mapping.chromeProfileId} onClick={()=>runTest(platform)}><MonitorCog size={14}/>{browserBusy===platform?'Đang kiểm tra…':'Kiểm tra'}</button></div>})}</div>
