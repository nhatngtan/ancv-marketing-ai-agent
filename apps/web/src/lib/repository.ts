...
const demoContents: ContentRecord[] = [
  {
    ...
    createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), createdBy: 'demo-user', status: 'partially_published',
    platforms: [
  ...
  },
  {
    ...
    createdBy: 'demo-user', status: 'review', platforms: [
...
export async function testConnector(platform: 'ga4' | 'search_console' | 'website', url?: string) {
  const backendUrl = import.meta.env.VITE_BACKEND_URL;
  const token = await auth?.currentUser?.getIdToken();
  if (!backendUrl || !token) throw new Error('Cần đăng nhập để chạy feasibility test.');
  const response = await fetch(`${backendUrl}/connectors/test`, {
    method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify(platform === 'website' ? { platform, url } : { platform }),
  });
  const result = await response.json() as { status?: string; message?: string };
  if (!response.ok) throw new Error(result.message ?? 'Feasibility test thất bại.');
  return result;
}
...
export async function fetchBackendHealth() {
  ...
  return response.json() as Promise<{ status: string; checkedAt: string; dependencies: Record<string, string> }>;
}
